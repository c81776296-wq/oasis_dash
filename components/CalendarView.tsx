            ...
            <div key={i} className={`min-h-[140px] p-2 border-r border-b border-gray-100 dark:border-gray-800 transition-colors hover:bg-gray-50/30 dark:hover:bg-gray-900/30 group relative flex flex-col ${!isCurrentMonth ? 'bg-gray-50/50 dark:bg-gray-900/50 opacity-40' : ''}`}>
              ...
              <div className="space-y-1 overflow-hidden">
                {dayTasks.map(task => (
                  <div
                    key={task.id}
                    className={`px-2 py-1 rounded border-l-2 text-[10px] font-semibold truncate transition-all shadow-sm ${task.status === 'COMPLETED' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border-green-500' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border-blue-500'}`}
                  >
                    {task.title}
                  </div>
                ))}
